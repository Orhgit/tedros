# Community Needs — Deep Dive

**Date**: 2026-04-27
**Author**: Vega (research synthesis from 12 sources, see end)
**Status**: Phase 0.5 — input to Vision v2.

A structured map of what an Ethiopian-Israeli community member actually needs across the life journey, grounded in published statistics, government audits, and the ecosystem of existing service organizations. This document supersedes the narrow "real-estate first" framing of the original Phase 0 summary by widening the scope to **every domain a community member touches** — while keeping real-estate as the primary monetization vehicle so the broader portal is sustainable.

## Population baseline

- **177,600** people (CBS, end of 2024) — ~1.76% of Israel's population. ([CBS]) ([WP-EJI])
- **53%** Ethiopia-born; **47%** Israel-born (second generation).
- **Geographic concentration**: Center 39% (Netanya 12.2K leads), South 24%; Kiryat Malakhi has the highest community share at 15.1%.
- **Local-authority representation**: <1.4% of municipal employees nationwide despite 2% population share. ([State Comptroller 2024])

## Cross-cutting findings

### Education gap

- High-school diploma eligibility: **36.8%** community vs **50.8%** general Jewish population — a 14-point gap, despite near-equal exam-taking rates (70.8% vs 71%). ([Knesset MMM])
- **10%** community youth haven't completed 12 years of schooling (vs 8.5% general). ([Knesset MMM])
- Rising school dropout, juvenile delinquency, depression, suicide cases since late 1990s. ([Humanium])

### Welfare load

- **41%** of community youth registered with welfare services (vs ~14% general).
- **22%** face poverty; **61%** face poor parental/child functioning. ([Knesset MMM])

### Employment & income

- Substantially lower wages and higher unemployment vs general Jewish population.
- Concentrated in low-wage sectors with limited advancement.
- Despite 2008 government decision and follow-up plans, public-sector senior representation remains minimal. ([JCPA], [Adva Center])
- **Olim Beyahad** has placed >700 academics with 90% retention; this is the *exception* that proves the systemic problem.

### Mental health

- Higher psychiatric hospitalization rates than general population. ([Springer Nature])
- Severe shortage of Amharic-fluent providers compounds stigma. ([NIH PMC])
- **Practical gap**: no public directory of Amharic-speaking therapists, social workers, or psychiatrists.

### Domestic violence

- Complaint rate significantly higher than population share. Murders by spouse spiked 2001–2006.
- Cultural transition disrupts traditional family-role structure → spousal tension. ([BIU Gender Studies], [Nogafem])
- Community-led empowerment networks (recognized 2022 Facebook community of the year) exist but operate informally.

### Law enforcement

- **3.5%** of police cases opened against community members (2019–2022) vs **1.7%** population share — exactly **2x** the rate. ([Calcalist])
- Complaints to מח"ש (Police Internal Investigations) at **4.5%** of all complaints (2017–2020) — **2.6x** population share. ([State Comptroller 7302])
- No body consolidates community police-conduct complaints; many are closed for "complainant abandonment."

### Real estate (existing pillar)

- **Mortgage 600K, 0% for 10 years**: 800 families/cycle eligible; lottery enrollment 70 NIS at participating bank branches. Active. ([Ynet]) Verified in original discovery doc.
- **Urban renewal**: 5 priority neighborhoods with cultural-bridge mandate (Rehovot, Rishon, Netanya).
- **Aliyah housing 2026**: First "direct absorption" pilot — Falash Mura olim placed directly into housing rather than klita centers. ([Ynet 2026])

### Heritage, religion, culture

- **Sigd** — central community holiday, 29 Cheshvan (Beta Israel covenant renewal); recognized state holiday.
- **Kessim** state-recognized 2018 as religious leadership; two yeshivas (Ashkelon, Beersheba). ([Hidabroot])
- **Center for Heritage** preserving oral → written tradition; primary academic node. ([ethiopianjhc.org.il])
- **Falash Mura aliyah**: govt did NOT fund continuation in 2025 budget — political risk for new immigrants. ([Knesset 03/2025])

## Needs across the life journey

The needs below are the matrix Tedros must serve. Each cell is a *mini-product* the platform should expose: read it, claim it, contact someone about it.

### 1. New immigrant (years 1–3 from aliyah)

| Need | Who serves it today | Tedros angle |
|---|---|---|
| Klita-center directory | Jewish Agency (5 centers stood up for Tzur Israel ops) | Aggregate + Amharic content + reviews |
| Hebrew-language learning | אולפן + Ministry of Aliyah | Find-an-ulpan + community ulpan reviews |
| Direct-absorption housing program | Govt 2026 pilot | Status checker, eligibility guide |
| Klita basket / education bag | Kol-Zchut (encyclopedic) | "Apply now" wizard + locale switch |
| Amharic-speaking pediatrician / GP | Tene Briut (informal) | Searchable directory |
| Kessim & community synagogue locator | Council of Kessim (no central site) | Map + service-request form |

### 2. Youth & education (ages 6–25)

| Need | Who serves it today | Tedros angle |
|---|---|---|
| Tutoring (matriculation gap) | ENP, PERACH, Fidel | Matchmaker + community mentor signup |
| University scholarships | ISEF, Hesegim, Kol-Zchut, individual unis | Single scholarship-finder with deadline alerts |
| Pre-army mechinot tailored | Tabor, Dror, Chosen | Comparison + application calendar |
| Career counseling (high school → workforce) | Olim Beyahad, Aharai | Personalized recommendation engine |
| Hi-tech entry programs | Tech-Career, Net4U, Coding Academy | Cohort calendar + alumni stories |
| Youth leadership | Mimad, Friends of Nature seeds | Events + applications |

### 3. Working-age adults (25–50)

| Need | Who serves it today | Tedros angle |
|---|---|---|
| Job board (community-aware) | Olim Beyahad placement | Curated listings; employer accountability |
| Promotions & advancement programs | Olim Beyahad excellence-track | Cohort applications + alumni stories |
| Small-business loans | UJIA-KIEDF | Eligibility wizard + lender contacts |
| Entrepreneurship training | Kurat, Shiluvim | Cohort calendar |
| Real-estate (sale/rent/urban renewal) | Yad2/Madlan (general); Tedros (community) | **Existing pillar** — community-aware listings + lead routing to vetted brokers |
| 600K mortgage — eligibility & application | Govt + community brokers | Wizard + status tracker + branch finder |
| Urban-renewal status (am I in a project?) | None unified | Address lookup → project + timeline + your rights |
| Public housing | Ministry + municipalities | Eligibility wizard + waitlist info |

### 4. Family & social

| Need | Who serves it today | Tedros angle |
|---|---|---|
| Daycare subsidies | Govt | Eligibility wizard |
| Domestic-violence resources | WTB list, community SWs | **Crisis-mode UX** (one-tap, anonymous, multi-lingual) |
| Women's empowerment networks | Facebook community + community SWs | Events + safe-space directory |
| Couples counseling (culturally aware) | Tene Briut + community SWs | Searchable directory |
| Mental health (Amharic-fluent) | Tene Briut + Kol-Zchut | **Anchor partnership content** — Amharic provider map |
| Anti-racism complaint filing | מח"ש (broken pipeline per Comptroller) | Guided complaint flow + record-keeping |
| Police-conduct legal aid | Tebeka | One-click triage to Tebeka with case prep |

### 5. Senior (50+) and second-generation parents

| Need | Who serves it today | Tedros angle |
|---|---|---|
| National Insurance rights (גיל זהב) | Bituach Leumi | Eligibility wizard with cultural framing |
| Long-term care (Amharic-fluent providers) | None unified | Provider directory |
| Heritage transmission to grandchildren | Heritage Center, IAEJ | Storytelling-archive + classroom resources |
| Synagogue + kessim accessibility | Council of Kessim | Locator + service-request |
| Pension & savings programs | Govt | Eligibility wizard |

### 6. Heritage, culture, identity (cross-cutting)

| Need | Who serves it today | Tedros angle |
|---|---|---|
| Sigd planning + events | IAEJ + Heritage Center | Event calendar + city editions |
| Amharic-language preservation | School-level | Resources hub + Ge'ez learning |
| Oral history archives | Heritage Center | Co-publish stories |
| Community news + activism | IAEJ + scattered Facebook | Aggregated news feed + RSS |
| Visibility / representation tracking | State Comptroller report | Live dashboard: representation by ministry |

### 7. Voice & advocacy (cross-cutting)

| Need | Who serves it today | Tedros angle |
|---|---|---|
| Report racism / discrimination | Govt anti-racism unit (no enforcement power per Comptroller) | Structured submission with auto-routing |
| Petitions + campaigns | IAEJ ad-hoc | Hosted campaign feature |
| Personal stories | Scattered | Curated user-submitted stories with consent flow |

## Strategic implications

### A. The thesis

Tedros's competitive position is **the Action Layer for an Information Ecosystem**.
- Information already exists across Kol-Zchut, ENP, Tebeka, Tene Briut, govt sites.
- Trust already lives at Tene Briut, Tebeka, ENP.
- What is missing: the unified surface where reading turns into doing — applying, booking, listing, complaining, finding a person — in HE/EN/AM.

### B. Three-layer product model

Every pillar exposes the same three layers:

1. **Read** — Programmatic SEO content, multi-lingual, sourced. Wins ranking by being more useful per page than Kol-Zchut's encyclopedia format.
2. **Do** — Wizards, applications, booking, complaint-filing, status-checking. Wins users by saving them the *Hebrew-bureaucracy-tax* that the community pays disproportionately.
3. **Connect** — Vetted professionals, mentors, peer support, kessim, klita centers. Wins trust by curating community-from-within.

### C. Why the existing real-estate-first framing is too narrow

- Real estate alone won't acquire users in 90 days. SEO authority on real-estate is contested (Yad2, Madlan, Komo) and slow.
- Information pillars (rights, scholarships, programs) acquire users **faster** because the SEO competition is weaker (Kol-Zchut is encyclopedic; we are action-driven).
- A wider portal *funds* itself through real-estate leads (existing plan) AND professional referrals AND optional employer postings — a portfolio of income streams instead of a single-track risk (R8: lead model unvalidated).

### D. Three "trust anchors" to land before public launch

- **Tene Briut** — owns community-trusted health voice. Lock in Amharic-provider-directory content collaboration.
- **Tebeka** — owns community-trusted legal voice. Lock in complaint-filing routing partnership.
- **ENP or Olim Beyahad** — owns community-trusted education/employment voice. Co-branded scholarship + job feeds.

Without at least Tene Briut and Tebeka in place, any health/legal content we publish risks looking like another well-meaning generalist site.

## Open questions

These move into the interview protocol (now needing expansion beyond real estate):

1. Which life-journey stage is most *underserved* today, in the user's lived experience? (Hypothesis: information-to-action gap on rights claims.)
2. For mental health, would the user prefer an *Amharic-fluent* provider or a *culturally fluent* one? (Could be different.)
3. Is there appetite for an anonymous racism/police-conduct reporting flow on Tedros, or does the user prefer Tedros stay neutral and direct them to Tebeka?
4. What language preference dominates *navigation* vs *content*? (Hypothesis: HE-first navigation, AM-first content for elders, EN-first content for diaspora visitors.)

## Sources

- **CBS** — Ethiopian-origin population end of 2024 ([CBS](https://www.cbs.gov.il/he/mediarelease/DocLib/2024/371/11_24_371b.pdf))
- **Wikipedia** — [ישראלים יוצאי אתיופיה](https://he.wikipedia.org/wiki/ישראלים_יוצאי_אתיופיה), [Ethiopian Jews in Israel](https://en.wikipedia.org/wiki/Ethiopian_Jews_in_Israel)
- **Knesset Research and Information Center** — [Education integration MMM](https://fs.knesset.gov.il/globaldocs/MMM/c0cd81db-49cd-e911-80fe-00155d0a9536/2_c0cd81db-49cd-e911-80fe-00155d0a9536_11_13510.pdf)
- **State Comptroller** — [Local-authority integration audit 2024](https://library.mevaker.gov.il/sites/DigitalLibrary/Documents/2024/Shilton/2024-Shilton-202-Shiloov.pdf), [Law enforcement conduct toward community](https://library.mevaker.gov.il/sites/DigitalLibrary/Pages/Reports/7302-7.aspx)
- **Brookdale Institute** — [20-year retrospective survey](https://brookdale-web.s3.amazonaws.com/uploads/2018/01/615-12-Ethiopian-Vatikim-HEB-REP-1.pdf)
- **Springer Nature / NIH PMC** — [Mental Health and Care Utilization](https://link.springer.com/article/10.1007/s40615-023-01557-x)
- **Humanium** — [Ethiopian Jewish immigrant children's rights](https://www.humanium.org/en/examining-the-rights-of-ethiopian-jewish-immigrant-children-in-israel/)
- **JCPA** — [Absorption of the Ethiopian community](https://jewishpublicaffairs.org/press-release/absorption-of-ethiopian-community-in-israel/)
- **Adva Center** — [Ethiopian-Israeli women employment 2021](https://adva.org/wp-content/uploads/2021/08/ethiopian-israeli-women-employment.pdf)
- **BIU Gender Studies** — [Domestic violence in the community](https://gender.biu.ac.il/node/1284)
- **Nogafem** — [Tene Briut feature](https://www.nogafem.org/product-page/טנא-בריאות-העמותה-לקידום-בריאות-של-יוצאי-אתיופיה-בישראל)
- **Government program** — [Integration plan landing](https://www.gov.il/he/departments/integration_program/govil-landing-page), [3242+3243 continuation 2025-2026](https://www.gov.il/he/pages/news-combination300725)
- **Calcalist** — [Police case rate analysis](https://www.calcalist.co.il/local_news/article/rjhyblpjt)
- **Ynet** — [Sigd explainer](https://www.ynet.co.il/articles/0,7340,L-5632698,00.html), [Direct-absorption housing pilot](https://www.ynet.co.il/judaism/article/sjfxmeer2)
- **Tebeka** — [tebeka.org.il](https://www.tebeka.org.il)
- **Tene Briut** — [tene-briut.org.il](https://tene-briut.org.il/)
- **ENP** — [enp.org.il](https://www.enp.org.il/he/)
- **Olim Beyahad** — [olim-beyahad.org.il](http://www.olim-beyahad.org.il/)
- **Heritage Center** — [ethiopianjhc.org.il](https://ethiopianjhc.org.il/)
- **Knesset 2025 budget press** — [Aliyah continuation defunded](https://main.knesset.gov.il/News/PressReleases/pages/press11032025s.aspx)
