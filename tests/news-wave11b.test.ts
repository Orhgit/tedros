// Wave 11b integrity guard (TED-163) — education / employment.
//
// This is the domain where the repo's worst fabrications were found (TED-152,
// TED-157): scholarships naming organizations that do not exist, invented
// thresholds, an ordinary means-tested benefit relabelled as a community-only
// entitlement. So beyond the usual slug/locale guards, this file pins the two
// things that would have caught those: every body names its sources, and no
// body cites a domain that was retired as never-registered.
...
import { ARTICLES_WAVE11B } from "../app/lib/news/articles-wave11b.server";
...
describe("wave 11b shape", () => {
  it("ships 8 articles", () => {
    expect(ARTICLES_WAVE11B).toHaveLength(8);
  });

  it("every wave 11b slug is unique across ALL waves", () => {
    const all = ARTICLES.map((a) => a.slug);
    expect(new Set(all).size).toBe(all.length);
    for (const a of ARTICLES_WAVE11B) {
      expect(all.filter((s) => s === a.slug)).toHaveLength(1);
    }
  });

  it("every wave 11b article is reachable through the main ARTICLES registry", () => {
  ...
  it("uses only valid tags, and stays inside the education/employment domain", () => {
    const domain = ["education", "employment"];
    for (const a of ARTICLES_WAVE11B) {
...
describe("wave 11b sourcing (ADR-021)", () => {
...
describe("wave 11b anti-fabrication guards (TED-152 / TED-157)", () => {
  it("cites no domain that was retired as never-registered", () => {
    // tech-career.org is live and is the address the government report names;
    // the .org.il variant never resolved. The others are from the TED-157 sweep.
    const retired = [
      "tech-career.org.il",
      "scaleup-velocity.org",
      "merom.org.il",
      "brachafund.org.il",
      "shalemfund.org.il",
    ];
    for (const a of ARTICLES_WAVE11B) {
      for (const locale of LOCALES) {
        for (const bad of retired) {
          expect(
            a.bodies[locale].includes(bad),
            `${a.slug}.${locale} cites retired domain ${bad}`,
          ).toBe(false);
        }
      }
    }
  ...
  it("keeps every shekel figure in a body that also links a source", () => {
    for (const a of ARTICLES_WAVE11B) {
      const he = a.bodies.he;
      if (!/₪|ש"ח/.test(he)) continue;
      expect(
        /\]\(https?:\/\//.test(he),
        `${a.slug} states an amount with no linked source`,
      ).toBe(true);
    }
  });
});
