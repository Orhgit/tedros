...
describe("PROGRAMME constants", () => {
  ...
  it("matches the official two-phase rate schedule (0% / 10y, 2% / 15y)", () => {
    ...
    expect(PROGRAMME.phase1.years + PROGRAMME.phase2.years).toBe(
      PROGRAMME.termYears,
    );
  });
});
...
describe("calculateEligibility — each IneligibilityReason is reachable", () => {
  ...
  it("single_parent_no_eligible_child: single parent with no child <21 in household", () => {
    ...
    expect(r.eligible).toBe(false);
    if (!r.eligible)
      expect(r.reasons).toContain("single_parent_no_eligible_child");
  });
...
describe("calculateEligibility — accumulation", () => {
  ...
  it("orders reasons by the canonical INELIGIBILITY_REASONS sequence", () => {
    ...
    if (!r.eligible) {
      const expected = (
        ["not_ethiopian_origin", "not_a_family", "owned_property_within_10y"] satisfies
          IneligibilityReason[]
      ).slice();
      expected.sort(
        (a, b) =>
          INELIGIBILITY_REASONS.indexOf(a) - INELIGIBILITY_REASONS.indexOf(b),
      );
