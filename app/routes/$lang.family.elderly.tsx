// /:lang/family/elderly — Senior Welfare rights hub.
// Old-age allowance (קצבת זקנה), long-term care insurance (סיעוד),
// day centres, transport allowance for 75+.
// JSON-LD: BreadcrumbList + WebPage.

import { Link } from "react-router";

import type { Route } from "./+types/$lang.family.elderly";
...
const HERO_IMAGE =
  "https://images.unsplash.com/photo-1598122666068-59b41e0a3193?fm=webj&q=70&w=1200&fit=crop";

export async function loader({ params }: Route.LoaderArgs) {
  const locale: Locale = isLocale(params.lang) ? params.lang : DEFAULT_LOCALE;
  const { PUBLIC_URL } = getEnv();

  const topic = getFamilyTopic("elderly");
...
export const meta: Route.MetaFunction = ({ data }) => {
  ...
  return [
    ...
    ...hreflangMeta(publicUrl, locale, "/family/elderly"),
    ...
    { property: "og:locale", content: locale },
    {
      "script:ld+json": webPageJsonLd(
        { publicUrl, locale },
        { path: "/family/elderly", name: title, description: subtitle },
      ),
    },
    {
      "script:ld+json": breadcrumbJsonLd({ publicUrl, locale }, [
        ...
        { name: title, path: "/family/elderly" },
      ]),
    },
  ];
};

export default function ElderlyPage({ loaderData }: Route.ComponentProps) {
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main id="main-content" className="container-default mx-auto max-w-4xl py-10">
        ...
        >
          {[
            {
              figure: locale === "he" ? "חינם" : locale === "am" ? "ነፃ" : "Free",
              label:
                locale === "he"
                  ? "תחבורה ציבורית לבני 75+"
                  : locale === "am"
                    ? "ህዝባዊ ትራንስፖርት ለ75+"
                    : "Public transport for 75+",
            },
            {
              figure: "₪1,800+",
              label:
                locale === "he"
                  ...
                  : locale === "am"
                    ? "ወርሃዊ የዕድሜ አበል"
                    : "Monthly old-age allowance",
            },
            {
              figure: locale === "he" ? "16 שע׳" : locale === "am" ? "16 ሰዓ" : "16 hrs",
              label:
                locale === "he"
                  ? "עזרה ביתית שבועית (סיעוד)"
                  : locale === "am"
                    ? "ሳምንታዊ የቤት ድጋፍ (ሰርቪስ)"
                    : "Weekly home assistance (care)",
            },
          ].map((stat) => (
            <div
              key={stat.label}
            ...
            >
              <p className="font-display text-3xl font-bold text-earth-800">{stat.figure}</p>
        ...
        <section
          ...
          aria-labelledby="elderly-info-heading"
        >
          <h2
            id="elderly-info-heading"
            className="mb-4 font-display text-xl font-semibold text-earth-900"
          >
            {locale === "he"
              ? "זכויות ותמיכה מפורטות"
              : locale === "am"
                ? "ዝርዝር መብቶች እና ድጋፍ"
                : "Detailed rights and support"}
          </h2>
        ...
        {/* Resources */}
        <section className="mb-10" aria-labelledby="elderly-resources-heading">
          <h2
            id="elderly-resources-heading"
            className="mb-4 font-display text-xl font-semibold text-earth-900"
          >
        ...
        {/* Bituach Leumi note */}
        <div className="mb-10 rounded-xl border border-earth-200 bg-earth-50 p-5">
          <h3 className="font-display text-base font-semibold text-earth-900">
            {locale === "he"
              ? "חשוב לדעת"
              : locale === "am"
          ...
          <p className="mt-2 text-sm text-ink-700">
            {locale === "he"
              ? "קצבת זקנה וביטוח סיעודי אינם משולמים אוטומטית — יש להגיש תביעה לביטוח לאומי. ניתן להגיש בסניף הקרוב, בטלפון *6050, או דרך האתר btl.gov.il."
              : locale === "am"
                ? "የዕድሜ አበል እና የሰርቪስ ዋስትና ቢቱዓ ሌዑሚ ጥያቄ ሳይቀርቡ አይከፈሉም። ጥያቄ ማቅረብ ይቻላል — ቅርብ ቅርንጫፍ፣ *6050፣ ወይም btl.gov.il።"
                : "Old-age allowance and long-term care insurance are not paid automatically — you must submit a claim to Bituach Leumi. Applications can be made in person, by phone at *6050, or via btl.gov.il."}
          </p>
        </div>
