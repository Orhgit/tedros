// Long-form urban-renewal prose — SERVER ONLY.
//
// Split out of ./registry.ts under ADR-020. Project status, authority,
// community context, qualitative unit notes and source lists are narrative
// content in three locales; the browser must never pay for all three.
//
// Loaders call `hydrateNeighborhood`/`hydrateNeighborhoods` and return the
// full record as loader data. Route `meta` functions then read it off
// `data` — `meta` CANNOT import this module, because `meta` is not stripped
// from the client build and the build fails outright.

import type {
  LocalizedText,
  UrbanRenewalNeighborhood,
  UrbanRenewalNeighborhoodIndex,
} from "./registry";
...
export const NEIGHBORHOOD_CONTENT: Record<string, NeighborhoodContent> = {
  ...
  "kibbutz-galuyot-kiryat-malakhi": {
    status: {
...
const EMPTY: LocalizedText = { he: "", en: "", am: "" };

/** Re-join an index entry with its prose. Server-side only. */
export function hydrateNeighborhood(
  n: UrbanRenewalNeighborhoodIndex,
): UrbanRenewalNeighborhood {
  const c = NEIGHBORHOOD_CONTENT[n.slug];
  return {
    ...n,
    units: { ...n.units, ...(c?.unitsNote ? { note: c.unitsNote } : {}) },
    status: c?.status ?? EMPTY,
    authority: c?.authority ?? EMPTY,
    communityContext: c?.communityContext ?? EMPTY,
    sources: c?.sources ?? [],
  };
}

export function hydrateNeighborhoods(
  list: UrbanRenewalNeighborhoodIndex[],
): UrbanRenewalNeighborhood[] {
  return list.map(hydrateNeighborhood);
}
