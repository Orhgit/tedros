...
export function parseEligibilityForm(
  ...
  for (const field of REJECTED_LEGACY_FIELDS) {
    if (raw[field] !== undefined && raw[field] !== "") {
      legacyErrors[field] = `Field "${field}" is no longer accepted by the eligibility check.`;
    }
  }
  ...
  const coerced = {
    ...
    hasChildUnder21LivingWithApplicant: toBool(
      raw.hasChildUnder21LivingWithApplicant,
    ),
