// Long-form city prose — SERVER ONLY.
//
// Split out of ./registry.ts under ADR-020: narrative copy in all three
// locales is bundle weight the browser must never pay for. Routes read it in
// their loader and hand the single resolved locale to the component.
//
// NOTE: a route `meta` function CANNOT import this module — `meta` is not
// stripped from the client build and the build fails outright. Resolve the
// strings in `loader` and read them off `data` in `meta` instead.
...
/** Narrative overview for a city, in the requested locale (Hebrew fallback). */
export function cityOverview(slug: string, locale: Locale): string {
  const o = CITY_CONTENT[slug]?.overview;
  return o?.[locale] ?? o?.he ?? "";
}

/** Community statistics for a city; empty when none are recorded. */
export function cityCommunityStats(slug: string): CommunityStat[] {
  return CITY_CONTENT[slug]?.communityStats ?? [];
}

/** Community stat values, locale-independent, positional (see generateCityFaq). */
export function cityStatValue(slug: string, idx: number): string {
  return CITY_CONTENT[slug]?.communityStats?.[idx]?.value ?? "";
}
