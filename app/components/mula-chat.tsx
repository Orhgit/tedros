  ...
  return (
    <>
      ...
      >
        ...
        <div
          className="relative flex flex-col items-center gap-1 overflow-hidden pb-3 pt-4"
          style={{ background: "linear-gradient(180deg, #0f0a1f 0%, #1a1035 60%, #2d1a5e 100%)" }}
          data-mula-scene
        >
          {/* Shimmer overlay */}
          <div className="absolute inset-0 opacity-30"
            style={{ background: "linear-gradient(135deg, transparent 40%, rgba(251,191,36,0.15) 50%, transparent 60%)", backgroundSize: "200% 100%", animation: "shimmer 4s ease-in-out infinite" }}
          />

          <MulaDigitalHuman state={avatarState} size={140} />

          <div className="relative flex items-center gap-2 pb-1">
            <p className="font-bold text-white text-base leading-tight">מולה</p>
            <span className="rounded-full bg-amber-500/30 px-2 py-0.5 text-[10px] font-semibold text-amber-300 ring-1 ring-amber-500/40">AI</span>
          </div>
          <p className="relative text-xs text-amber-200/70 -mt-1 pb-1">
            {avatarState === "thinking" ? "⋯ חושב" : avatarState === "talking" ? "▶ מדבר" : "עוזר קהילתי · Tedros"}
          </p>

          <button
            onClick={() => setOpen(false)}
            aria-label="סגור"
            className="absolute top-3 left-3 rounded-full p-1 text-white/50 hover:bg-white/10 hover:text-white transition-colors"
          >
        ...
        {/* ── Messages ── */}
        <div className="flex flex-col gap-3 overflow-y-auto px-4 py-4" style={{ maxHeight: "280px", minHeight: "120px" }}>
          {messages.map((msg, i) => (
            <div key={i} className={cn("flex gap-2 items-end", msg.role === "user" ? "flex-row-reverse" : "flex-row")}>
              ...
              <div className={cn(
                ...
                msg.role === "user" ? "bg-amber-500 text-white rounded-br-sm" : "bg-gray-100 text-gray-800 rounded-bl-sm",
              )}>
                {msg.content}
              </div>
            </div>
            ...
            <div className="flex items-end gap-2">
              ...
              <div className="rounded-2xl rounded-bl-sm bg-gray-100 px-4 py-3 shadow-sm">
                <span className="flex gap-1.5">
                  {[0, 1, 2].map((d) => (
                    <span key={d} className="size-2 rounded-full bg-amber-400"
          ...
          {error && <p className="rounded-lg bg-red-50 border border-red-100 px-3 py-2 text-center text-xs text-red-500">{error}</p>}
          <div ref={bottomRef} />
        </div>
        ...
        <div className="flex items-center gap-2 border-t border-gray-100 bg-gray-50/60 px-3 py-3">
          ...
          />
          <button onClick={() => sendMessage(input)} disabled={loading || !input.trim()} aria-label="שלח"
